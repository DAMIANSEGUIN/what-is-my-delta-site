# Deployment Enforcement Access Note
**Created:** 2025-11-16 @ 22:48 local  
**Context:** Replacement for `NOTE_FOR_CURSOR_AND_CLAUDE_CODE_2025-10-27.md` for agents who cannot open the legacy note.
...
## Codex Update – 2025-11-17

- `.githooks/` now contains **pre-commit**, **pre-push**, and **post-commit** hooks, so running `./scripts/setup_hooks.sh` keeps all three enforcement layers active. Legacy copies remain under `.git/hooks/` for reference but are no longer used once `core.hooksPath` is set.
- The post-commit (GDrive sync) hook now allows environment overrides (`GDRIVE_PROJECT_DIR`, `GDRIVE_SYNC_REMOTE`, `GDRIVE_RCLONE_BIN`) and gracefully skips when required paths are unavailable, preventing failures on other machines.
- Emergency bypasses **must** include `BYPASS_REASON`. Attempts to set `SKIP_VERIFICATION=true` without a reason will now fail in both the wrapper script and tracked pre-push hook, ensuring audit entries stay meaningful.

Next actions remain unchanged: clean the working tree, rerun `./scripts/pre_push_verification.sh`, exercise the wrapper push, and schedule the GitHub Action test once secrets are available.
